export const editInputClass =
  "bg-bgHighlight/60 border border-borderSoft/80 rounded-md px-2 py-1 " +
  "text-textPrimary placeholder:text-textSecondary/60 " +
  "hover:bg-bgHighlight/70 hover:border-borderSoft " +
  "focus:outline-none focus:bg-bgHighlight/80 focus:border-accent " +
  "focus:ring-1 focus:ring-accent/40 transition";

export const labelClass = "text-xs text-textSecondary uppercase tracking-wide";

export const rowClass =
  "grid grid-cols-12 gap-3 items-end rounded-xl border border-borderSoft bg-bgMain/40 px-4 py-3";

export const cellClass = "flex flex-col gap-1";
