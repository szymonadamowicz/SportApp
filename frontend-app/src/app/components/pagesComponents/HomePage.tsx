import {
  nextUp,
  recentHighlights,
  tipForTheDay,
  trainings,
  weeklyProgress,
} from "@/mocks/HomePageMocks";
import InfoPanel from "../InfoPanel";

export default function HomePage() {
  return (
    <>
      <InfoPanel title="Are you ready for training xyz?"  items={nextUp} />

      <div className="columns-1 md:columns-2 gap-6">
        <div className="break-inside-avoid inline-block w-full mb-6">
          <InfoPanel
            title="Trainings left this week"
            anchorDesc={{
              label: "View Plan",
              href: "/workouts",
            }}
            items={trainings}
            dimOthers={true}
          />
        </div>

        <div className="break-inside-avoid inline-block w-full mb-6">
          <InfoPanel title="Tips for the day" items={tipForTheDay.tip} />
        </div>

        <div className="break-inside-avoid inline-block w-full mb-6">
          <InfoPanel
            title="Week progress"
            desc={`sessions ${
              trainings.filter((i) => i.workout?.completed).length
            }/${trainings.length}`}
            progress={
              trainings.filter((i) => i.workout?.completed).length /
              Math.max(1, trainings.length)
            }
            items={weeklyProgress}
            layout="row"
            maxPerRow={3}
          />
        </div>

        <div className="break-inside-avoid inline-block w-full mb-6">
          <InfoPanel
            title="Recent highlights"
            anchorDesc={{ label: "See all", href: "/profile" }}
            items={recentHighlights}
            layout="row"
            maxPerRow={3}
          />
        </div>
      </div>
    </>
  );
}
